...
const formBuilder = (friend) => {
  const form = `
    <div class="form-group">
      ...
      <input type="text" class="form-control" value="${friend.name}" id="form-friend-name" placeholder="Monkey-butt">
    </div>
    <div class="form-group">
      <label for="form-friend-address">Address:</label>
      <input type="text" class="form-control" value="${friend.address}" id="form-friend-address" placeholder="400 Monkey Ln">
    </div>
    <div class="form-group">
      <label for="form-friend-email">Email:</label>
      <input type="email" class="form-control" value="${friend.email}" id="form-friend-email" placeholder="monkeybutt@aol.com">
    </div>
    <div class="form-group">
      <label for="form-friend-phone">Phone #:</label>
      <input type="text" class="form-control" value="${friend.phoneNumber}" id="form-friend-phone" placeholder="867-5309">
    </div>
    <div class="form-group">
      <label for="form-friend-relationship">Relationship:</label>
      <input type="text" class="form-control" value="${friend.relationship}" id="form-friend-relationship" placeholder="Brother">
    </div>
  `;
  return form;
};
...
const showEditForm = (e) => {
  ...
  friendsData.getSingleFriend(idToEdit)
    .then((singleFriend) => {
      ...
      domString += `<button id="edit-friend" data-single-edit-id=${singleFriend.id}>Save Changes</button>`;
      $('#add-edit-friend').html(domString).show();
      $('#friends').hide();
    })
    .catch((error) => {
...
const updateFriend = (e) => {
  const friendToUpdate = gettingFriendFromForm();
  const friendId = e.target.dataset.singleEditId;
  friendsData.updateFriend(friendToUpdate, friendId)
    .then(() => {
      $('#add-edit-friend').html('').hide();
